...
p=8;  % why 24 months?   Recall AR(2p+) "folklore" for AR(p) signal in noise
...
plot(1:p,b,'d'); box off; for j=1:p, line([j j],a(j,:)); end
 ...
 nmc=10000;   % number of sampled "futures"
 ...
 figure(2); clf
 boxplot(bsamp'); ylabel('AR(8) coefficients \phi_j'); 
...
p=8;  % how to think about choice of p? 
 ...
 figure(2); clf;
        decomp_plot_sim(x,sdecomp,4); eval(xa)
        
        
npred=12;    % number of future time points
 nmc=10000;   % number of sampled "futures"
 x=origY(iy,:)';
 [b,B,r,nu,v]=ar(x,p);
 [xpred,bsamp,vsamp] = arsim(x,p,b,B,nu,v,nmc,npred);
 ...
 % plot some of the sampled futures .. clearly now
 plot(1:n, x); hold on; 
 plot((n):(n+npred), [x(n);median(xpred, 2)]);
 plot((n):(n+npred), [x(n);prctile(xpred, 25, 2)]); 
 plot((n):(n+npred), [x(n);prctile(xpred, 75, 2)]); 
 plot((n):(n+npred), [x(n);prctile(xpred, 10, 2)]); 
 plot((n):(n+npred), [x(n);prctile(xpred, 90, 2)]);
 hold off;
