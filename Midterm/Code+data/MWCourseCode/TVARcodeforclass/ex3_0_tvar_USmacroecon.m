...
% --------------- Read in and transform data: 
    ...
    %
    [Y,Ynames]=xlsread('o2Forward.xlsx');
    [T q]=size(Y); q=q-1; zq=zeros(q,1); oq=ones(q,1); Iq=eye(q);
    yrqtr=Y(:,1); Y(:,1)=[]; Y=Y'; Ynames(1)=[]; names=char(Ynames);
    %
...
pause
m0=zeros(p,1); m0(1)=1; n0=5; s0=0.02; C0=eye(p)/2; % initial priors %?? m0=zeros(p,1); m0(1)=1; n0=5; s0=0.02; C0=eye(p)/2
del=[0.99 .975];  %  AR and variance discount factors %?? 0.99 0.975
% 
...
ic=min(4,size(waves,1))      % select 3 quasi-prediodic components if they exist #??4?
...
prBigger = zeros(T,1);
prBiggerCI = zeros(T,2);
ratios = zeros(T,1);
ratioCI = zeros(T,2);
samples = 1000;
score = tinv([0.025  0.975],samples-1)
for time = 1:T
    p1samples = datasample(mods(1,time,:), samples);
    p2samples = datasample(mods(2,time,:), samples);
    ratio = p1samples ./ p2samples ;
    thing = ratio(1,1,:);
    ratios(time) = mean( thing );
    temp = score * std( thing ) / samples;
    ratiosCI(time,1) = ratios(time) - temp(1);
    ratiosCI(time,2) = ratios(time) + temp(1);
    thing(thing<1) = 0;
    thing(thing>1) = 1;
    prBigger(time) = mean(thing);
    temp = score * std( thing ) / samples;
    prBiggerCI(time,1) = prBigger(time) + temp(1);
    prBiggerCI(time,2) = prBigger(time) + temp(2);
    time
end
    
figure(4); clf
    subplot(2,1,1)
    plot(1:T,ratios(1:T,1),'color','k')
    axis tight; ylim([0 1.1]); box off; eval(xa)
    ylabel('ratio'); title(['Modulus ratio']) 
    hold on; xline(466); 
    line(1:T, ratiosCI(:,1));
    line(1:T, ratiosCI(:,2));
    hold off;
    ...
    subplot(2,1,2)
    line(1:T,prBigger(1:T,1),'color','k')
    axis tight; ylim([0 1.1]); box off; eval(xa)
    ylabel('Probability'); title(['Probability that mod1 > mod 2']) 
    hold on; xline(466); hold off;
...
for i = 1:I
    i
    maxIdx(i) = find(Y(:, i)== maxStats(1, i));
    minIdx(i) = find(Y(:, i)== minStats(1, i));
end
...
subplot(2,1,1); 
    histogram(maxIdx);
    ylabel('count'); title(['Maximum achieved index statistics'])
subplot(2,1,2);     
    histogram(minIdx);
    ylabel('count'); title(['Minimum achieved index statistics'])

meanMax = mean(maxStats)
varMax = var(maxStats)

meanMin= mean(minStats)
varMin = var(minStats)
