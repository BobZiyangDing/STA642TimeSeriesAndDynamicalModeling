...
fprintf('Hit a key to start analysis ...\n\n')
pause
% 
...
deltrend=0.9; delregn=0.98; delseas=0.95; delvar=1; % 0.98; % variance discount factor
...
for t=1:T
    ...
    if (t>1) a = G*m; 
             R = G*C*G'; R(itrend,itrend)=R(itrend,itrend)/deltrend;  
                         ...
                         nu=delvar*nu; % variance discounting
    end
    %Fregn=X(:,t)'; F(iregn)=Fregn; 
...
%         sq=tinv(.95,nu);  
%         h=sqrt(squeeze(sC(iregn,iregn,1:t)))'*sq; 
%         ciplot(sm(iregn,1:t)-h,sm(iregn,1:t)+h,1:t,[.85 .85 .85]); hold on
